...
# live in .context/ and are served to coding agents over MCP. Editing the
# Markdown in .context/knowledge by hand is expected and supported; this script
...
if [ -d .context ]; then
  echo "refusing to overwrite the existing .context; remove it first" >&2
  exit 1
fi

$CLI init >/dev/null

cat > .context/config.yaml <<'YAML'
...
remember "The Markdown in .context/knowledge is the only source of truth" \
  --type rule --scope core.store --enforcement must --pin \
  --body "The in-memory index and .context/.cache/index.json are derived. Both can be deleted at any time and must be rebuilt from the Markdown, never the other way round. A developer editing a knowledge file by hand is a supported workflow, not an edge case."
...
remember "Agents may only stage proposals, never accept them" \
  ...
  --body "The MCP server deliberately exposes no accept, edit or delete tool. An agent can write to .context/proposals and nowhere else, so a human always decides what the project believes. Adding a tool that mutates .context/knowledge directly would break the core product promise."

remember "All knowledge writes go through ChronicleStore" \
  --type rule --scope core --enforcement must \
  --body "Never touch files under .context/knowledge with fs directly. The store owns id generation, slug uniqueness, atomic writes, archive moves, the index and the history log; bypassing it desynchronises all five."
...
One Markdown file per knowledge item under .context/knowledge, with YAML frontmatter, committed to Git.
...
## Consequences
Queries need an index, so the store keeps an in-memory map rebuilt from disk and cached, keyed by mtime, in the gitignored .context/.cache."
...
remember "Tests live in packages/*/test and run with Vitest from the repo root" \
  ...
  --body "The root vitest.config.ts aliases @chronicle/core to its source, so tests run against the current source without building first. Store and MCP tests work against a real temporary .context directory rather than mocking the filesystem."
