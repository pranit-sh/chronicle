...
export function createChronicleServer(options: ChronicleServerOptions = {}): McpServer {
  ...
  const server = new McpServer(
    { name: "chronicle", version: options.version ?? "0.1.0" },
    {
      instructions: [
        ...
        "",
        "Proactively call context_resolve before planning, writing or changing code. Do not wait",
        "for the developer to ask for context. Pass the file you are about to touch and a short",
        "description of the task. It returns only the knowledge relevant to that spot, which is far",
        "more reliable than guessing conventions from the surrounding code.",
        "",
        "As the conversation unfolds, decide whether information from the developer is durable",
        "project knowledge: a rule, decision, convention, architectural fact, domain concept, current",
        "context note or known issue that future agents should know. If it is worth keeping, call",
        "knowledge_propose yourself; do not wait for the developer to say remember, record or add it.",
        "The proposal still waits for human review before it becomes shared project knowledge.",
        "",
  ...
  server.registerTool(
    "context_resolve",
    {
      ...
      description:
        "Return the project knowledge that applies to a specific file and task: the rules, decisions, architecture, domain concepts, conventions, current context and known issues in scope. Call this proactively before planning or editing code; do not wait for the developer to ask for context.",
  ...
  server.registerTool(
    "knowledge_propose",
    {
      ...
      description: [
        ...
        "edits or rejects.",
        "",
        "Use this instead of private assistant memory whenever the conversation yields durable",
        "project knowledge: a rule, decision, convention, architectural fact, domain concept,",
        "current context note or known issue that future agents should know. Do not wait for",
        "explicit wording such as \"remember to\", \"record\", \"always use\", \"never use\",",
        "\"prefer\" or \"avoid\"; decide whether the information is worth proposing and stage it",
        "when it is.",
        "",
