...
All notable changes to the Chronicle extension are documented here.
...
- **Context view** — the exact package an agent would receive for the file you
  ...
  budget and why.
- **Guide view** — the Chronicle workflow, available actions, verification check
  syntax and agent setup.
- Setup commands for the MCP server in Copilot, Cursor and Claude Code, plus a
  command to add Chronicle instructions to your agent instruction files.
...
- Status bar entry showing how many knowledge items apply to the current file,
  toggled by `chronicle.statusBar.enabled`.
- `chronicle.groupBy` setting to group the knowledge tree by type, status or
  scope.

[0.1.0]: https://github.com/pranit-sh/chronicle/releases/tag/v0.1.0
